...
const CATEGORY_REPORT_TARGET_TYPES = ['All', 'POST', 'COMMENTARY'];
...
const CategoryReports = ({categorySlug}) => {
    ...
    const isAuthModerator = isModerator();

    const isModeratorForThisCategory = isAuthModerator || (Array.isArray(moderatedCategorySlugs) && moderatedCategorySlugs.includes(categorySlug));

    const lastReportRef = useCallback(
        node => {
            if (loading || initialLoading || !hasMore || !isModeratorForThisCategory) return;
        ...
        [loading, initialLoading, hasMore, isModeratorForThisCategory]
    );

    const fetchReports = useCallback(async () => {
        if (!isModeratorForThisCategory || userLoading || loadingModeratedCategories) {
        ...
        try {
            if (reportId.trim()) {
                const res = await axios.get(`http://localhost:8080/api/v1/categories/slug/${categorySlug}/reports/${reportId.trim()}`, {
            ...
            if (axios.isAxiosError(err)) {
                if (err.response?.status === 404 && reportId.trim()) {
                    console.log(`Report with ID ${reportId.trim()} not found.`);
                    fetchedReports = [];
                    isLast = true;
                } else if (err.response?.status !== 404 && err.response?.status !== 500 && err.response?.status !== 403) {
                    if (page > 0 && !reportId.trim()) {
                        toast.error(errorMessage);
                    } else if (page === 0 && !reportId.trim()) {
                        toast.error(errorMessage);
                    } else if (reportId.trim()){
                        toast.error(errorMessage);
                    }
                    fetchedReports = [];
                    isLast = true;
                } else if (err.response?.status === 403 && page === 0 && !reportId.trim()) {
                    console.log("Permission denied to fetch category reports.");
                    fetchedReports = [];
                    isLast = true;
                } else {
                ...
                console.error("An unexpected error occurred during report fetching:", err);
            ...
            setTimeout(() => {
                setReports(prev => (page === 0 && !reportId.trim() ? fetchedReports : reportId.trim() ? fetchedReports : [...prev, ...fetchedReports]));
                setHasMore(!isLast && !reportId.trim());
    ...
    useEffect(() => {
            const delay = page === 0 && !reportId.trim() ? 100 : 0;
            const handler = setTimeout(() => {
                fetchReports();
            }, delay);
            return () => clearTimeout(handler);
    }, [reportId, selectedStatus, selectedReason, selectedTargetType, sortOption, page, fetchReports]);
        ...
        return (
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans text-gray-900">
                ...
                <p className="text-center text-gray-500 py-10 text-base">
                    You do not have permission to view reports for this category.
                </p>
            </div>
        );
    }
        ...
        <div className="mx-auto font-sans text-gray-900">
            <div className="flex flex-wrap gap-x-8 gap-y-6 mb-10">
                ...
                {[
                    ...
                    ['Target Type', CATEGORY_REPORT_TARGET_TYPES, selectedTargetType, handleTargetTypeChange],
            ...
            {initialLoading && reports.length === 0 ? (
                ...
                <p className="text-center text-gray-500 py-10 text-base">
                    {reportId.trim()
                        ? `Report with ID "${reportId}" not found in this category.`
                        : 'No reports found matching your criteria in this category.'}
                </p>
            ) : (
                <div className="space-y-6">
                    {reports.map((report, i) => (
                        <div key={report.id} ref={!reportId.trim() && i === reports.length - 1 ? lastReportRef : null}>
            ...
            {loading && page > 0 && !reportId.trim() && (
                <div className="flex justify-center py-6">
                    <Oval height={40} width={40} color="#6B7280" secondaryColor="#E5E7EB" strokeWidth={3} visible />
                </div>
            )}

            {hasMore && reports.length > 0 && !reportId.trim() && (
                <p className="text-center text-gray-500 text-sm mt-6 pb-4">
